Import-Module $PSScriptRoot\..\PSWordPress.psd1 -Force

# Build credentials object for automation or simply do Get-Credential
$SecurePassword = ConvertTo-SecureString -String ''
$Credentials = [System.Management.Automation.PSCredential]::new('PowerShell', $SecurePassword)
# Authorize to Wordpress
$Authorization = Connect-Wordpress -Credential $Credentials -Url 'https://evotec.xyz/'

# Cmdlets for Tags
$SpecificTags = Get-WordpressTag -Authorization $Authorization -Id 50, 885 -Verbose
$SpecificTags | Format-Table

$List = Get-WordpressTag -Authorization $Authorization -Verbose
$List | Format-Table

$ListSpecific = Get-WordpressTag -Authorization $Authorization -Verbose -Search 'powershell'
$ListSpecific | Format-Table

# Cmdlets for category
$SpecificTags = Get-WordPressCategory -Authorization $Authorization -Id 66, 792 -Verbose
$SpecificTags | Format-Table

$List = Get-WordPressCategory -Authorization $Authorization -Verbose
$List | Format-Table

$ListSpecific = Get-WordPressCategory -Authorization $Authorization -Verbose -Search 'powershell'
$ListSpecific | Format-Table