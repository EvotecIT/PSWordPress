Import-Module $PSScriptRoot\..\PSWordPress.psd1 -Force

# Build credentials object for automation or simply do Get-Credential
$SecurePassword = ConvertTo-SecureString -String ''
$Credentials = [System.Management.Automation.PSCredential]::new('PowerShell', $SecurePassword)
# Authorize to Wordpress
$Authorization = Connect-Wordpress -Credential $Credentials -Url 'https://evotec.xyz/'

# List pages (default 10) with specifc tags
$List = Get-WordPressPage -Authorization $Authorization -Verbose -RecordsPerPage 20
$List | Format-Table

# List posts (default 10) with specifc tags
$List = Get-WordPressPage -Authorization $Authorization -Search 'psteams' -Verbose
$List | Format-Table

$ActualPost = Get-WordPressPage -Authorization $Authorization -id 12089
$ActualPost | Format-Table