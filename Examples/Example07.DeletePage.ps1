# Build credentials object for automation or simply do Get-Credential
$SecurePassword = ConvertTo-SecureString -String ''
$Credentials = [System.Management.Automation.PSCredential]::new('PowerShell', $SecurePassword)
# Authorize to Wordpress
$Authorization = Connect-Wordpress -Credential $Credentials -Url 'https://evotec.xyz/'

$NewPage = New-WordPressPage -Authorization $Authorization -Verbose -Title 'This is a page to create' -Content '<p> This is content with large </p>' -WhatIf

# Lets update the post we just created
if ($NewPage.Id) {
    # List post we just added
    $List = Get-WordPressPage -Authorization $Authorization -Verbose -Id $NewPage.id -Context edit
    $List | Format-Table id, date, modified, status, title, content

    # Update post
    Set-WordPressPage -Authorization $Authorization -Id $NewPage.Id -Title 'Changing Title to other to delete' -WhatIf

    # List post we just added
    $List = Get-WordPressPage -Authorization $Authorization -Verbose -Id $NewPage.id
    $List | Format-Table id, date, modified, status, title, content

    Remove-WordPressPage -Authorization $Authorization -Id $NewPage.Id -WhatIf
}