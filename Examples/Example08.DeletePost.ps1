Import-Module $PSScriptRoot\..\PSWordPress.psd1 -Force

# Build credentials object for automation or simply do Get-Credential
$SecurePassword = ConvertTo-SecureString -String ''
$Credentials = [System.Management.Automation.PSCredential]::new('PowerShell', $SecurePassword)
# Authorize to Wordpress
$Authorization = Connect-Wordpress -Credential $Credentials -Url 'https://evotec.xyz/'

$NewPost = New-WordPressPost -Authorization $Authorization -Verbose -Title 'This is a post to create' -Content '<p> This is content with large </p>' -WhatIf

# Lets update the post we just created
if ($NewPost.Id) {
    # List post we just added
    $List = Get-WordPressPost -Authorization $Authorization -Verbose -Id $NewPost.id -Context edit
    $List | Format-Table id, date, modified, status, title, content

    # Update post
    Set-WordPressPost -Authorization $Authorization -Id $NewPost.Id -Title 'Changing Title to other' -WhatIf

    # List post we just added
    $List = Get-WordPressPost -Authorization $Authorization -Verbose -Id $NewPost.id
    $List | Format-Table id, date, modified, status, title, content

    Remove-WordPressPost -Authorization $Authorization -Id $NewPost.Id -WhatIf
}